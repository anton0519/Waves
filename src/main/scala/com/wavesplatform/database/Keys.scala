...
object Keys {
  ...
  private def hAddr(prefix: Short, height: Int, addressId: BigInt): Array[Byte] = {
    val addressIdBytes = addressId.toByteArray
    ByteBuffer.allocate(6 + addressIdBytes.length).putShort(prefix).putInt(height).put(addressIdBytes).array()
  }
  ...
  def leaseBalance(height: Int, addressId: BigInt): Key[LeaseBalance] =
    Key(hBytes(13, height, addressId.toByteArray), readLeaseBalance, writeLeaseBalance)
  ...
  def addressTransactionIds(height: Int, addressId: BigInt): Key[Seq[(Int, ByteStr)]] =
    Key(hBytes(20, height, addressId.toByteArray), readTransactionIds, writeTransactionIds)
  ...
  def addressScript(height: Int, addressId: BigInt): Key[Option[Script]] =
    Key.opt(hBytes(28, height, addressId.toByteArray), ScriptReader.fromBytes(_).explicitGet(), _.bytes().arr)
