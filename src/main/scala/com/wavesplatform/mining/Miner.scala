  ...
  private def forgeBlock(account: PrivateKeyAccount, balance: Long): Either[String, (MiningConstraints, Block, MiningConstraint)] = {
    ...
    measureSuccessful(
      ...
      for {
        ...
        _ <- checkDelay(height, account.publicKey, refBlockBT, balance, blockDelay)
          .map(validBlockDelay => {
            log.debug(
        ...
        block <- Block
          .buildAndSign(version.toByte, currentTime, referencedBlockInfo.blockId, consensusData, unconfirmed, account, blockFeatures(version))
          .leftMap(_.err)
      } yield (estimators, block, updatedMdConstraint.constraints.head)
    )
  }

  private def checkDelay(height: Int, accountPublicKey: Array[Byte], refBlockBT: Long, balance: Long, blockDelay: Long): Either[String, Long] = {
    pos
      .getValidBlockDelay(height, accountPublicKey, refBlockBT, balance)
      .leftMap(_.toString)
      .ensure(s"${System.currentTimeMillis()}: Block delay $blockDelay was NOT less than estimated delay")(_ < blockDelay)
  }
